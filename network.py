...
if __name__ == "__main__":
    ...
    trainset = CIFAR10(root="./data/", transform=transform, train=False)

    datld = DataLoader(trainset, batch_size=64, shuffle=False)
    images, labels = next(iter(datld))
    ...
    # If it's a list/tuple, grab the first element (as your traceback shows)
    ...
    # batch = next(iter(datld))

    # print(images.shape, batch)
    print(torch.max(model.forward(images), dim=1)[1] == labels)
